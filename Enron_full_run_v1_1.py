...
targets = ['dynegy', 'apache', 'steele', 'cochise', 'chewco', 'whitewing']
...
for subdir, dirs, files in os.walk(rootdir):
    for file in files:
        ...
        if (not (sent_only =="Y" or sent_only =="y")) or ("sent" in subdir):
            ...
            # print(message_subject_handled, "   ", message_date, "   ", message_from, "   ", message_to)
            ...
            if message_to:
                ...
                for to_item in message_to_list:
                    ...
                    edges_from_to_list.append([message_subject_handled, message_date, message_from, to_item]) 
                ...
                for ent in message_body_nlpd.ents:
                    ...
                    edges_subject_entity_list.append([message_subject_handled, message_date, message_from, ent, ent.label_])
                    
                # Loop on target entities or phrases to find    
                for target in targets:
                    if target in message_subject_handled.lower() or target in message_body.lower():
                        edges_subject_target_list.append([message_subject_handled, message_date, message_from, target])

            # If no message_to then log file in emails_not_processed list
            else:
                emails_not_processed.append(filepath)
